	...
	render() {
		const columns = [
			...
			{ title: 'Column 8', dataIndex: 'address', key: '8' },
			{
				...
				width: 100,
				render: () => (
					<a onClick={this.showFlowModifyModal} href="javascript:;">
						FlowModify
					</a>
				),
			},
		...
		const columns2 = [
			...
			{ title: 'Column 8', dataIndex: 'address', key: '8' },
			{
				...
				width: 100,
				render: () => (
					<a onClick={this.showFileModifyModal} href="javascript:;">
						FileModify
					</a>
				),
			},
			...
			<div className="mdcConfig" style={{ height: '100%' }}>
				...
				<div className="right_content">
					<div className="card-container" style={{ height: '100%' }}>
						...
						<Icon
							className="mdcAddIcon"
							onClick={this.showMdcAddModal}
							type="plus"
						/>
						<div className="panel">
							<div className="warn">
								...
								；MDC实例停止标识：
								<span className="stopDot" />
								；MDC状态在停用状态或非工作时段，允许修改配置，其他情况不允许修改配置；
							</div>
							...
							<div className="operate">
								<Button
									type="primary"
									onClick={this.showSrcAddModal}
									className="operate_btn"
								>
									添加输入行情源
								</Button>
								<Button type="primary" className="delete_btn">
									删除输入行情源
								</Button>
							</div>
							<Table
				...
				<ConfigModal
					onClose={this.configModalClose}
					show={this.state.configModalShow}
				/>
				<MdcAddModal
					onClose={this.mdcAddModalClose}
					show={this.state.mdcAddModalShow}
				/>
				<SrcAddModal
					onClose={this.srcAddModalClose}
					show={this.state.srcAddModalShow}
				/>
				<FlowModifyModal
					onClose={this.flowModifyModalClose}
					show={this.state.flowModifyModalShow}
				/>
				<FileModifyModal
					onClose={this.fileModifyModalClose}
					show={this.state.fileModifyModalShow}
				/>
			</div>
		)
	}
